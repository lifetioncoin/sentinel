...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, lifetioncoind):
        golist = lifetioncoind.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_lifetioncoind(lifetioncoind, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_lifetioncoind(self, lifetioncoind, rec):
        import decimal
        ...
        # exclude any invalid model data from lifetioncoind...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from lifetioncoind, with every run
        ...
        # get/create, then sync payment amounts, etc. from lifetioncoind - lifetioncoind is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(lifetioncoind)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from lifetioncoind! %s" % e)
            govobj.vote_delete(lifetioncoind)
            return (govobj, None)
    ...
    def vote_delete(self, lifetioncoind):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(lifetioncoind, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, lifetioncoind, signal, outcome):
        ...
        output = lifetioncoind.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = lifetioncoinlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(lifetioncoind, signal)

    def sync_network_vote(self, lifetioncoind, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = lifetioncoind.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = LIFETIONCOIND_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 lifetioncoin addr, non-multisig
            if not lifetioncoinlib.is_valid_lifetioncoin_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid lifetioncoin address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # LifetionCoin Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (lifetioncoinlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = LIFETIONCOIND_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not lifetioncoinlib.is_valid_lifetioncoin_address(addr, config.network):
    ...
    def hash(self):
        import lifetioncoinlib
        return lifetioncoinlib.hashit(self.serialise())
