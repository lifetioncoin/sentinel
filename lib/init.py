...
def has_lifetioncoin_conf():
    import config
    import io

    valid_lifetioncoin_conf = False

    # ensure lifetioncoin_conf exists & readable
    #
    # if not, print a message stating that LifetionCoin Core must be installed and
    # configured, including JSONRPC access in lifetioncoin.conf
    try:
        f = io.open(config.lifetioncoin_conf)
        valid_lifetioncoin_conf = True
    except IOError as e:
        print(e)

    return valid_lifetioncoin_conf
...
def main():
    ...
    if not has_lifetioncoin_conf():
        print("LifetionCoin Core must be installed and configured, including JSONRPC access in lifetioncoin.conf")
        sys.exit(1)
