...
class LifetionCoinConfig():
    ...
    def slurp_config_file(self, filename):
        # read lifetioncoin.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is lifetioncoin.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        # get rpc info from lifetioncoin.conf
        ...
        default_port = 9555 if (network == 'mainnet') else 19555

        # use default port for network if not specified in lifetioncoin.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from lifetioncoin.conf
