...
def is_valid_lifetioncoin_address(address, network='mainnet'):
    ...
    lifetioncoin_version = 127 if network == 'testnet' else 48
    ...
    try:
        ...
        # rescue from exception, not a valid LifetionCoin address
        return False

    if (address_version != lifetioncoin_version):
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('lifetioncoin.conf')
