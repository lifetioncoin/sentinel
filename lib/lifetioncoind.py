"""
lifetioncoind JSONRPC interface
"""
import sys
import os
...
class LifetionCoinDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some lifetioncoind methods
    ...
    @classmethod
    def from_lifetioncoin_conf(self, lifetioncoin_dot_conf):
        from lifetioncoin_config import LifetionCoinConfig
        config_text = LifetionCoinConfig.slurp_config_file(lifetioncoin_dot_conf)
        creds = LifetionCoinConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = lifetioncoinlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = lifetioncoinlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = lifetioncoinlib.blocks_to_seconds(diff)
